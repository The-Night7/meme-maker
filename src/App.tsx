...
export default function App() {
  ...
  const [pendingCaptions, setPendingCaptions] = useState<any>({});
  const [showModerationPanel, setShowModerationPanel] = useState(false);
  const [potentiallyInappropriateWords, setPotentiallyInappropriateWords] = useState<any>({});
  ...
  const createRoom = async () => {
    ...
    const initialData = {
      hostId: user.uid,
      status: 'lobby',
      players: {}, // L'hôte n'est plus ajouté aux joueurs !
    ...
    try {
      ...
      // Redirection immédiate vers son tableau de bord
      navigate(`/admin/${code}`);
    } catch (err) {
                                  ...
                                  return (
                                    <li key={idx} className="bg-gray-900 p-2 rounded-lg">
                                      ...
                                      <div className="mt-1 text-xs text-red-400">
                                        Mots détectés: {inappropriateWords.map((w: any) => w.word).join(', ')}
                                      </div>
                                    </li>
                                  );
                                })}
